...
class Cache:
    def __init__(self, **kwargs) -> None:
        data_path = kwargs["data_path"] if "data_path" in kwargs else "cache"
    ...
    def write_unscrobbled(self, unscrobbled_list):
        try:
            for record in unscrobbled_list:
                if record.trackid is None or record.artist is None or record.title is None or record.last_observation_timestamp is None:
                    continue
                self.db_cache_cursor.execute('''
